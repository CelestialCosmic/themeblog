...
这篇文章基于大量前人研究、开源仓库、英伟达官方文档、个人见闻与个人实践
...
但现在连 AI 架构都没有稳定下来的情况下，任何尝试去做通用 agent 的行为都是**炮灰行为**：
1. AI 架构日新月异，一旦改组意味着推倒重来
2. 现有的 agent 完成简单操作仍然需要人工干预，在复杂操作中直接趴窝
3. 功能实现不是产品开发工作中最主要的一环。从立项提需求、数轮测试到上生产环境并运维，在应用的全生命流程中，任何一项都比功能实现要耗时

## 并不是只有 deepseek
DeepSeek 这类是多模态大模型，或者说解析能力是 Any
Hugging Face 根据模型的类型分为如下几大类：
1. 计算机视觉
2. 自然语言处理
3. 音频
4. 图表
5. 图学习
多模态拥有上面数个 / 全部能力，但专业处理依旧是数个小模型围着一个大模型，各司其职
...
## 其他 AI 能做的事情
AI 会颠覆很多成熟商业公司的产品
比如 AI+OCR ：[GitHub - opendatalab/MinerU: A high-quality tool for convert PDF to Markdown and JSON.一站式开源高质量数据提取工具，将PDF转换成Markdown和JSON格式。](https://github.com/opendatalab/MinerU)
...
```sh
ollama create DeepSeek-R1-Distill-Qwen-32B -f ./modelfile
```
