做了几天功课，显卡到了，装上，把线理清楚（没想到库存的两根 hdmi 一根 DP 一根不稳定，两根坏了，把卡装上加点亮用了半个小时，理线反而用了一个小时），收尾完立刻最快速度点亮，上 deepseek
## 硬件条件
电脑配置：
1. cpu:amd 9950X
...
在 ollama.cpp 中改了量化模式以后，16G 显存可以正常运行 Int4 的 32B
...
### 模型消失的问题
systemd 启动 ollama 和 `ollama serve` 启动使用不同的文件夹，要修改配置文件改一下，改 systemd 方便一些